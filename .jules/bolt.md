...
## 2026-03-10 - Replace read-modify-write patterns with atomic Prisma upserts
**Learning:** Performance optimizations should prioritize atomic database operations (e.g., Prisma `upsert`) where appropriate instead of the read-modify-write pattern (`findUnique` followed by `create` or `update`) to prevent unnecessary database round-trips and Time-of-Check to Time-of-Use (TOCTOU) race conditions. This is a common performance bottleneck across the codebase that causes N+1 queries.
**Action:** When working on database operations, always prefer atomic `upsert` over the read-modify-write pattern to improve performance and prevent race conditions.
