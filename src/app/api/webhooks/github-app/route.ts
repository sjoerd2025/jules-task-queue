...
async function handleInstallationRepositoriesEvent(
  ...
  if (action === "added") {
    ...
    await db.$transaction(async (prisma) => {
      // Optimized: Use a single updateMany with the `in` operator to avoid O(N) database queries
      await prisma.installationRepository.updateMany({
        where: {
          installationId: installation.id,
          repositoryId: { in: repositories.map((r: GitHubWebhookRepository) => BigInt(r.id)) },
        },
        data: { removedAt: new Date() },
      });
