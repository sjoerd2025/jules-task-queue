...
class GitHubAppClient {
  ...
  public async findInstallationForRepo(
    ...
    try {
      // Fast path: Check local database first to avoid N+1 API calls
      const dbInstallation = await db.installationRepository.findFirst({
        where: {
          owner,
          name: repo,
          removedAt: null,
      ...
      if (dbInstallation) {
        return dbInstallation.installationId;
      }

      // Fallback: If not found in DB, try GitHub API iteration
